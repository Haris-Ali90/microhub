...
use App\Http\Requests\Backend\LoblawsRequest;
use App\Walmart;
use DB;
...
class NewLoblawsController extends BackendController
{
    ...
    public function statistics_loblaws_index(Request $request)
    {
       ...
       if(empty($date))
       {
        ...
        return backend_view('newloblaws.statistics_loblaws_dashboard',compact('date'));
    }

    /**
     * Get Loblaws OTD graph
     */
    public function ajax_render_otd_charts(Request $request)
    {
        ...
        $fullrecord=Task::join('sprint__sprints','sprint__sprints.id','=','sprint__tasks.sprint_id')
            ->whereIn('sprint__sprints.creator_id', [477194,477195])->where(\DB::raw("CONVERT_TZ(FROM_UNIXTIME(sprint__tasks.eta_time),'UTC','America/Toronto')"), 'like', $date . "%")
    ...
    public function ajax_render_loblaws_orders(Request $request)
    {
        ...
        $html =  view('backend.newloblaws.sub-views.ajax-render-view-loblaws-orders',compact('fullrecord','page','total_page','total','otd'))->render();
    ...
    public function ajax_render_ota_charts(Request $request)
    {
       ...
       $fullrecord=Task::join('sprint__sprints','sprint__sprints.id','=','sprint__tasks.sprint_id')
        ...
        $fullrecord=Sprint::join('sprint__tasks','sprint__tasks.sprint_id','=','sprint__sprints.id')
            ...
            ->whereIn('sprint__sprints.creator_id', [477194,477195])->where(\DB::raw("CONVERT_TZ(sprint__sprints.created_at,'UTC','America/Toronto')"), 'like', $date . "%")
        ...
        return response()->json(array('status' => true,'for'=>'loblaws-ota',
            'data_set_one' =>
            //[60],
                $otas,
            'data_set_two' =>
    ...
    /**
     * Get Loblaws new order count
     */
    public function loblawsNewCount()
    {
        $current_date = date("Y-m-d H:i:s");
        $pervious_date = date('Y-m-d H:i:s',strtotime('-20 seconds',strtotime(date("Y-m-d H:i:s"))));
        $pervious_count = \Illuminate\Support\Facades\DB::table('sprint__sprints')->whereIn('creator_id', [477194,477195])->WhereNotIn('sprint__sprints.status_id', [35,36,37,38])->where('created_at', '<=', $pervious_date)->count();
        $new_count = \Illuminate\Support\Facades\DB::table('sprint__sprints')->whereIn('creator_id', [477194,477195])->WhereNotIn('sprint__sprints.status_id', [35,36,37,38])->where('created_at', '<=', $current_date)->count();
        return $new_count - $pervious_count;

    }

    /**
     * Loblaws Dashboard Order Reporting Csv View
     */
    public function loblaws_dashboard_csv_index()
    {
        return backend_view('newloblaws.loblaws_orders_report_view');
    }

    /**
     * Loblaws Dashboard Order Reporting Ajax Download
     */
    public function loblaws_dashboard_csv_download(LoblawsRequest $request)
    {
        ...
        $file_name = (isset($metaData['file_name'])) ? $metaData['file_name'] :'Loblaws Dashboard Order Report File '.date("M d, Y",strtotime($request->fromdatepicker)).'-('.uniqid().').csv';
        ...
        $fullrecord=Task::join('sprint__sprints','sprint__sprints.id','=','sprint__tasks.sprint_id')
            ->whereIn('sprint__sprints.creator_id', [477194,477195])
