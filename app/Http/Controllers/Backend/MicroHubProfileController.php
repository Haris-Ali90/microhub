...
class MicroHubProfileController extends BackendController
{
    public function getIndex()
    {
        ...
        $jc_user = DB::table('jc_users')->where('email_address',$user->email)->first();

        // foreach ($jc_user as $data){
            // $jc_user_data = $data->id;
            $jc_user_address = $jc_user->user_address;
            $jc_user_phone = $jc_user->user_phone;
        // }

        // dd($jc_user_data);

        $hub_request = HubRequest::where('jc_user_id',$jc_user->id)->first();
        // dd();
        $hub_user_data = (empty($hub_request)) ? 3 : $hub_request->own_joeys;
        // foreach ($hub_request as $data){
            // $hub_user_data = $data->own_joeys;
        // }

        return backend_view('profile.profile',compact('user','jc_user','hub_user_data','jc_user_address','jc_user_phone'));
    }
