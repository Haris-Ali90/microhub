...
//dd($wmorders);
    ...
    public function ajax_render_total_orders_summary(Request $request)
    {
        ...
        $query = "SELECT
                ...
                WHERE TYPE = 'pickup'
                AND sprint__sprints.creator_id IN (476734,475761,476610,476734,476850,476867,476933,476867,476967,476968,476969,476970,477006,477068,477069,477078,477123,477124,477150,477153,477154,477157,477192,477267,477268,477279)
    ...
    public function walmartNewCount()
    {
        ...
        $pervious_count = \Illuminate\Support\Facades\DB::table('sprint__sprints')->whereIn('creator_id', [476734, 475761, 476610, 476734, 476850, 476867, 476933, 476867, 476967, 476968, 476969, 476970, 477006, 477068, 477069, 477078, 477123, 477124, 477150, 477153, 477154, 477157, 477192, 477267, 477268, 477279])->where('created_at', '<=', $pervious_date)->count();
        $new_count = \Illuminate\Support\Facades\DB::table('sprint__sprints')->whereIn('creator_id', [476734, 475761, 476610, 476734, 476850, 476867, 476933, 476867, 476967, 476968, 476969, 476970, 477006, 477068, 477069, 477078, 477123, 477124, 477150, 477153, 477154, 477157, 477192, 477267, 477268, 477279])->where('created_at', '<=', $current_date)->count();
    ...
    public function generate_walmart_report_csv(WalmartRequest $request)
    {
        ...
        if(!isset($metaData['filter_ids']))
        {
            ...
            $sprint_ids = Task::join('sprint__sprints', 'sprint__tasks.sprint_id', '=', 'sprint__sprints.id')
                ->whereBetween(\DB::raw("CONVERT_TZ(FROM_UNIXTIME(due_time),'UTC','America/Toronto')"), [$from_date, $to_date])
                ->whereIn('sprint__sprints.creator_id', $walmart_vendors_ids)
                ->where('sprint__tasks.type', 'dropoff')
        ...
        // getting sprint data
        $sprint_data = Sprint::whereIn('id', $sprint_ids)
        ...
        foreach ($sprint_data as $key => $sprint) {
            ...
            $csv_row = [
                "CR-" . $sprint->id,
                $sprint->SprintLastDropOffTask->Merchantids->merchant_order_num,
                ConvertTimeZone($sprint->created_at, 'UTC', 'America/Toronto'),
                stripcslashes($sprint->SprintLastDropOffTask->task_Location->address),
                $sprint->store_num,
                $joey_name,
                $schedule,
                $compliant_pickup,
