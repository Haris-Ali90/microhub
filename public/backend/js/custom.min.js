...
// ---------------------------------------For pending and resubmit modules-------------------------------------------------------
function openStatusChangeModal(id){
	$('#openStatusChangeModal').modal('show');
	$('#openStatusChangeModal #claim_id').val(id);

}
function getReasons (identifier,route) {
	var slug= $( "#status_id option:selected" ).text();
	getreasonsAjax(slug,route);

}
function getreasonsAjax(slug,route) {
	$.ajax({
		type: 'POST',
		url: newurl+route,
		...
		success: function (res) {
			...
			if(res.length > 0){
				$('#reason_id')
					...
					.remove()
					.end()
					.append('<option selected disabled value="">Select a reason</option>')
					.val('')
				;
				res.forEach(appendOptions);
			}else{
				$('#reason_id')
					.find('option')
					.remove()
					.end()
					.append('<option value="">No Reasons Available For this Status Type</option>')
					.val('')
				;
			}
		}
...
function appendOptions(item, index) {
	$("#reason_id").append(new Option(`${item.title}`, `${item.id}`));
}

function uploadImageBtn(route) {
	// var form_data = new FormData();
	// var id=$('#hiddenIdImageUploadModal').val();
	// console.log($('#status_id').val());

	if($('#status_id').val()==null){
		alert("Status is required!");
	}
	if($('#reason_id').val()==null){
	...
	if (typeof document.getElementById('file').files[0] == "undefined") {
		...
		if ($.inArray(fileType, validImageTypes) < 0) {
			...
			//    alert("Image");
			// $("#file").val(null);
			// $('#hiddenIdImageUploadModal').val(null);
		}
	}
