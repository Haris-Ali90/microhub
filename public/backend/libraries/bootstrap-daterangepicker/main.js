$(function () {
    ...
    // --------------------- Common - [Start]
    var minenddate =  new Date('2021-02-02');
    ...
    function returnFileSize(number){
        if(number < 2048) {
            return number + 'bytes';
        } else if(number >= 2048 && number < 25600) {
            return true;
        } else if(number >= 25600) {
            return false;
        }
    };
    ...
    var COMMON = {
        validateSingleFile: function(thisFile){
            var filesize = thisFile[0];
            var isValid = validateSize(filesize);
            var thisUploadControl = thisFile.closest('.custom-upload-control');
            var thisempty = thisFile.closest('#file-uploader');

            if(isValid){
                if(!thisFile.val() && thisFile.attr('required')){
                    thisUploadControl.addClass('invalid');
                } else {
                    if(thisUploadControl.find('.input-expiry-date').length){
                        var expDateInput = thisUploadControl.find('.input-expiry-date');
                        if(expDateInput.val() || thisFile.val()){
                            var imgSrc = thisFile.closest('.upload-box').find('img').data('src');
                            if(isBlank(imgSrc) && !thisFile.val()){
                                thisFile.attr('required', 'required');
                                thisUploadControl.addClass('invalid');
                            }
                            if(!expDateInput.val()){
                                expDateInput.attr('required', 'required');
                            }
                            if(expDateInput.val() && !isBlank(imgSrc)) {
                                thisUploadControl.removeClass('invalid');
                                // thisUploadControl.append('<span style="float: left;">File uploaded,Maximum allowed File size is 2MB.</span>');
                            }
                        }
                ...
                alert('File size greater then 2MB!' +
                    'Please chose file less then 2MB');
                // thisUploadControl.append('<span style="float: left;">File size is invalid, maximum allowed File size is 2MB please try again</span>');
                thisUploadControl.addClass('invalid');
                // thisempty.refresh();
                thisUploadControl.addClass('invalid').closest('.upload-box').find('.uploaded-image-inner').removeClass('pdf-file').removeClass('doc-file');
                thisUploadControl.wrap('<input type="file" id="file-uploader">').closest('form').get(0).reset();
                thisUploadControl.unwrap();
        ...
        validateForm: function(thisElement, e){
            ...
            console.log('check props', form[0]);
            console.log('check', form[0].checkValidity());
        ...
        startDatePicker: function() {
            if($(SELECTOR.startDate).length){
                $(SELECTOR.startDate).datepicker({
                    ...
                    // $(SELECTOR.endDate).attr('value', endDate);
        ...
        validateQuiz: function(){
            $(SELECTOR.quizQuestionBox).each(function(){
                if($(this).find('[type="radio"]').is(':checked')){
                    $(this).find('.next-quiz-btn').show();
                } else {
                    $(this).find('.next-quiz-btn').hide();
                }
            })
        },
        checkQuiz: function(){
    ...
    // --------------------- Page Document - [Start]
    var PAGE_DOCUMENT = {
        validateOptionalDocuments: function(){
            // $(SELECTOR.)
        },
        init: function(){
