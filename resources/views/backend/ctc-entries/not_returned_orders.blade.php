...
@section('inlineJS')
    ...
    <script>
        ...
        $(function () {
            appConfig.set('yajrabox.ajax', '{{ route('new-notreturned-ctc.data') }}');
        ...
        function getTotalOrderData() {
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#hub_return_scan').text(data['ctc_count']['hub_return_scan']);
                    $('#hub_not_return_scan').text(data['ctc_count']['return_orders'] - data['ctc_count']['hub_return_scan']);
                    // hide loader
        ...
        <div class="">
            <div class="page-title">
                <div class="title_left">
                    <h3 class="">{{ $title_name }} Returns Not Received At Hub<small></small></h3>
                </div>
            </div>
...
{{--                            <h2>{{ $title_name }} <small>Returns Not Received At Hub</small></h2>--}}
                            @if(can_access_route('new-notreturned-ctc-export.excel',$userPermissoins))
                            <div class="excel-btn" style="float: right">
                                <a href="{{ route('new-notreturned-ctc-export.excel') }}"
                                   class="btn buttons-excel buttons-html5 btn-sm btn-primary excelstyleclass">
                                    Export to Excel
                                </a>
                            </div>
                            @endif
                            @if(can_access_route('new-notreturned-ctc-tracking-export.excel',$userPermissoins))
                                <div class="excel-btn" style="float: right">
                                    <a href="{{ route('new-notreturned-ctc-tracking-export.excel') }}"
                                       class="btn buttons-excel buttons-html5 btn-sm btn-primary excelstyleclass">
