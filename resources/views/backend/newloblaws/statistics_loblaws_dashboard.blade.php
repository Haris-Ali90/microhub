                    ...
                    <div class="x_title col-md-12">
                        ...
                        <div  class=" text-right">
                          <button  onclick="loblawsrefresh()"  class="btn btn-success pl-md-3 refresh" style="display: none">Refresh</button>
                            <button  target="_blank" onclick="exportTableToCSV('loblaws-reprt-<?php echo $date ?>.csv')"  class="btn btn-success pl-md-3">Export CSV</button></div>
                            <div class="clearfix"></div>
                        </div>
        ...
        $(document).ready(function () {
            ...
            setInterval(function () {
                ...
                if(CurretnDate == Selectdate) {
                    $.ajax({
                        type: "get",
                        url: '{{ URL::to('loblaws/new-count')}}',
                        data: {},
                        success: function (data) {
                            if (data >= 1) {
                                $('#notify').val(data);
                                toast.success('' + data + ' New Loblaws Order Created');
                            }
                        }
                    });
                }
    ...
    if (idleTime > 5) { // 5 minutes
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let el = $('.loblaws-statistics-checkbox');
                var url_for = 'loblaws-orders';//el.attr('data-for');
