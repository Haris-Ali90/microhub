...
@section('content')
        ...
        <div class="">
            ...
            <div class="page-title">
                <div class="title_left amazon-text">
                    <h3>Last Mile Routes<small></small></h3>
                </div>
            </div>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="x_panel">
                        <form id="filter" action="" method="get">
                            ...
                            <?php
                             $datas = auth()->user()->hub_id;
                            ?>
                            <div class="col-md-6 d-flex justify-content-end pr-0">
                                <button type='button' style="float: right" class='dselect btn btn green-gradient actBtn' data-toggle='modal' data-target='#ex50' id="rt" onclick='fullmap({{$datas}})' >Show Full Routes Map <i class='fa fa-map'></i></button>
                                <a href="{{ url('last/mile/zones') }}"  class='btn btn green-gradient actBtn'>Back To Main <i class='fa fa-list'></i></a>
                            </div>
                        </form>
                        <div class="x_content">
                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    for($j=0;$j<count($counts);$j++) {
                                        ...
                                        if($counts[$j]->joey_id){
                                            $joeyId = '('.$counts[$j]->joey_id.')';
                                        }else{
                                            $joeyId = $counts[$j]->joey_id;
                                        }
                                        echo "<td>" . $counts[$j]->first_name .' '. $counts[$j]->last_name .' '. $joeyId . "</td>";
    ...
    <div id="ex1" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                <form action="ctc/routes/add" method="post">
    ...
    <div id="details" class="modal"  role="dialog" aria-hidden="true">
        <div class="modal-body">
            <div class='modal-dialog'>
                <div class='modal-content'>
    ...
    <div class="toast active hide-toast">
    ...
    <script>
        ...
        function Routemap(data)
        {
            ...
            for (var i = 0; i < data['data'].length; i++) {
                for (var k = 0; k < data['data'][i].length; k++) {
                    ...
                    if(mapmarker==1)
                    {
                        var icon_marker="https://assets.joeyco.com/images/marker/marker_red"+(data['data'][i][k]['ordinal'])+".png";
                    }
                    else
                    {
        ...
        function fullmap(id) {
            ...
            $.ajax({
                url : '../../../../allroute/'+ id +'/location/joey?date=<?php echo $date; ?>',
                type : 'GET',
                dataType:'json',
