...
@section('inlineJS')
   ...
   $( function() {
    ...
    <script>
       ...
       /* $(function () {
            appConfig.set('yajrabox.ajax', '{{ route('directmontreal.data') }}');
            ...
            appConfig.set('yajrabox.columns', [
                ...
                {data: 'picked_hub_time', orderable: false, searchable: false, className: 'text-center'},
                {data: 'sorter_time', orderable: false, searchable: false, className: 'text-center'},
                {data: 'dropoff_eta', orderable: false, searchable: false},
                {data: 'delivery_time', orderable: false, searchable: false},
        ...
        <div class="">
            <div class="page-title">
                <div class="title_left amazon-text">
                    <h3 class="text-center">Montreal Dashboard<small></small></h3>
                </div>
            </div>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="x_panel">
                        <div class="x_title">
                            ...
                            @if(can_access_route('export_Montreal.excel',$userPermissoins))
                            <div class="excel-btn" style="float: right">
                                <a href="{{ route('export_Montreal.excel') }}"
                        ...
                        <div class="x_content">
                            ...
                            <div class="table-responsive">
                                <table   class="table table-striped table-bordered test ">
                                    ...
                                    <tbody>
                                    @foreach($query as $kry => $result)
                                        <tr> <td>{{$result->order_id}}</td>
                                            <td>{{$result->route}}</td>
                                            <td>{{$result->joey}}</td>
                                            <td>{{$result->address}}</td>
                                            <td>{{$result->picked_hub_time}}</td>
                                            <td>{{$result->sorter_time}}</td>
                                            <td>{{$result->dropoff_eta}}</td>
                                            <td>{{$result->delivery_time}}</td>
                                            <td>{{$result->image}}</td>
                                            <td>{{$result->tracking_id}}</td>
                                            <td>{{$result->sprint_status}}</td>
                                            <td>action</td>
