                    ...
                    <div class="x_panel">
                        <div class="x_title col-md-12">
                            ...
                            <div  class=" text-right">
                              <button  onclick="walmartRefresh()"  class="btn btn-success pl-md-3 refresh" style="display: none">Refresh</button>
                                <a  target="_blank" href={{ URL::to("report/export")}}?date=<?php echo $date ?> class="btn btn-success pl-md-3">Export Data</a></div>
                            <div class="clearfix"></div>
                        </div>
...
@section('inlineJS')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            setInterval(function () {
                ...
                //if(CurretnDate == Selectdate) {
                    $.ajax({
                        type: "get",
                        url: '{{ URL::to('walmart/new-count')}}',
                        data: {},
                        success: function (data) {
                            if (data >= 1) {
                                $('#notify').val(data);
                                toast.success('' + data + ' New Walmart Order Created');
                            }
                        }
                    });
               // }
        ...
        function walmartRefresh() {
          //  showLoader();
            let el = $('.wm-statistics-checkbox');
            var url_for = 'total-orders-summary';
            var status =  'false';
            let targeted_div = 'total-orders-summary-main-wrap';
...
/*
        ...
        $('.wm-statistics-checkbox').click(function () {
            ...
            if(status == 'false' && check_type == true ){
                ...
                if (url_for == 'total-orders-summary'){
                    $('.refresh').show();
                }
                //show loader
        ...
        $(document).on('click','.paginationbt',function(){
            ...
            $.ajax({
                type: "get",
                url: "{{ URL::to('dashboard/statistics/ajax/total-orders-summary')}}",
